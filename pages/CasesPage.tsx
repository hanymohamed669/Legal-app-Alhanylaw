...
import { CaseIcon, SearchIcon, PlusIcon } from '../components/icons';
...
const CasesPage: React.FC = () => {
    ...
    const fetchCases = () => {
         if (user) {
            setLoading(true);
            api.getCases(user, false).then(data => {
                setCases(data);
                setLoading(false);
            });
        }
    ...
    const handleArchive = async (caseId: string) => {
        if(window.confirm('هل أنت متأكد من رغبتك في أرشفة هذه القضية؟')) {
            await api.archiveCase(caseId, true);
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                {canEdit && (
                     <button className="flex items-center bg-primary text-white px-4 py-2 rounded-lg shadow hover:bg-primary-dark transition-colors">
                        <span className="ml-2">إضافة قضية جديدة</span>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                            ...
                            filteredCases.map(caseItem => (
                                <tr key={caseItem.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-left text-sm font-medium space-x-4">
                                        ...
                                        {canEdit && <button onClick={() => handleArchive(caseItem.id)} className="text-yellow-600 hover:text-yellow-800">أرشفة</button>}
                                    </td>
                                </tr>
                            ))
                        )}
                 ...
                 { !loading && filteredCases.length === 0 && (
                    <div className="text-center py-12">
                        <CaseIcon className="mx-auto w-12 h-12 text-gray-300" />
                        <h3 className="mt-2 text-sm font-medium text-gray-900">لم يتم العثور على قضايا</h3>
                        <p className="mt-1 text-sm text-gray-500">
                            {filters.searchTerm || filters.clientId || filters.keyword || filters.sessionDate || filters.caseYear ? "حاول تعديل بحثك." : "أضف قضية جديدة للبدء."}
                        </p>
                    </div>
                )}
            </div>
